...
/// [[MonthlyReviewGalleryView]] 的月卡背景：把当月的抠图贴纸从卡片顶上「倒进来」，
/// 走真物理 —— iOS 内置的 `UIDynamicAnimator` 挂 gravity + collision + itemBehavior，
/// 贴纸会掉、会撞、会转、会堆到卡片底边。
///
/// 卡片是 218pt × ~350pt 的小区域，跑完 ~6 秒动画就把引擎停掉，剩下静态叠加图，
...
final class PhysicsCutoutHostView: UIView {
    ...
    private let collision: UICollisionBehavior = {
        ...
        c.translatesReferenceBoundsIntoBoundary = true // 拿 self 的 bounds 当四壁
        return c
    }()
    ...
    override func layoutSubviews() {
        ...
        // bounds 第一次拿到真实值之前，别急着丢贴纸 —— 拿零尺寸 boundary 起手全掉出去了。
    ...
    func updateCutouts(_ cutouts: [UIImage]) {
        // 只关注新增：已经在物理里跑的不动它，稳定的月份不会因为父视图 rebuild 就重新掉一遍。
        let capped = Array(cutouts.prefix(maxStickers))
        pendingCutouts = capped
        if hasLaidOut { dropPending() }
    }
    ...
    /// 停引擎但保留 subview 的当前位置 —— 卡片进入静态期，节能。
    private func freeze() {
    ...
    private func spawn(_ image: UIImage) {
        ...
        // 起点：卡片顶上一点点、水平位置在 bounds 内的随机横坐标（贴纸中心为准）。
        let xRange = max(1, bounds.width - w)
        let startX = CGFloat.random(in: 0...xRange, using: &rng)
        let startY = -h - CGFloat.random(in: 10...80, using: &rng)
